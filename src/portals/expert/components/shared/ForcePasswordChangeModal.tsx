...
import { clearExpertSession } from '@expert/utils/auth';

export const PW_CHANGED_FLAG = 'expertPwChanged';

/**
 * Blocking dialog shown when the account was reset by an admin
 * (mustChangePassword). It cannot be dismissed — the expert must set a new
 * password. The backend revokes the session on success, so we clear local
 * tokens and send them back to the login page.
 */
export function ForcePasswordChangeModal() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setBusy(true);
    try {
      await expertApi.changePassword(currentPassword, newPassword);
      sessionStorage.setItem(PW_CHANGED_FLAG, '1');
      clearExpertSession();
    ...
    <>
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-900">비밀번호 변경 필요</h2>
